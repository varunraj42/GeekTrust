...
class Guest(Resident):
    ...
    def calc_water_capacity(self):
        return self.guest_count * super().supplycap_by_person

class WaterSupplyPricing():
    per_day_pricing_by_supply_type = {"Co-orporation": 1, "Borwell": 1.5, "Tanker": {"0-500": 2, "500-1500": 3, "1500-3000": 5, "3001-": 8}}
    # 4500 - 500 = 4000 (1000/-)
    # 4000 - 1000 = 3000 (3000/-)
    # 3000- 1500 = 1500 (7500/-)
    # 1500 (12000/-)

class Coorp(WaterSupplyPricing):
    def __init__(self, water_cap_required):
        self.water_cap_required = water_cap_required

    def calc_water_price(self):
        return self.water_cap_required * super().per_day_pricing_by_supply_type["Co-orporation"]
...
class Borewell(WaterSupplyPricing):
    ...
    def calc_water_price(self):
        return self.water_cap_required * super().per_day_pricing_by_supply_type["Borwell"]
...
class Tanker(WaterSupplyPricing):
    ...
    def calc_water_price(self):
        total_price = 0
        slab_prices = super().per_day_pricing_by_supply_type["Tanker"]
        ...
        for slab_cap in slab_prices:
            ...
            if slab_max_cap > 3000:
                ...
                print(f"Total prices found for next {self.water_cap_required} lit is {total_price}/-")
                ...
                print(f"Total prices found for next {slab_max_cap} lit is {total_price}/-")
...
# Residents ka Kharcha
    residents_per_day_wc = resident.calc_water_capacity()
    residents_monthly_wc = residents_per_day_wc * total_num_days
    print(f"Residents Water Capacity for a {apart.apartment_type}BHK Apartment for {total_num_days} days --> {residents_monthly_wc}")

    coorp_ws_req, borewell_ws_req = calc_resident_water_cap_by_ratio(residents_monthly_wc, coorp_ratio, borewell_ratio)
    print(f"Co-orp WS ({coorp_ratio} Parts): {coorp_ws_req}-lit vs Borewell WS ({borewell_ratio} Parts): {borewell_ws_req}-lit for a total supply of {residents_monthly_wc}-lit")

    coorp = Coorp(coorp_ws_req)
    monthly_coorp_bill = coorp.calc_water_price()
    print(f"Month End Co-orpoartion Bill: Rs {monthly_coorp_bill}/-")

    borewell = Borewell(borewell_ws_req)
    monthly_borewell_bill = borewell.calc_water_price()
    print(f"Month End Borwell Bill: Rs {monthly_borewell_bill}/-")
...
# Guests ka Kharcha
    ...
    print(f"Total number of guests: {final_guest_count}")

    guest_fam_wc = guest_fam.calc_water_capacity()
    print(f"Guest-Fam Water Capacity --> {guest_fam_wc}")

    guests_monthly_wc = guest_fam_wc * total_num_days
    print(f"Guests Water Capacity for {total_num_days} days --> {guests_monthly_wc}")

    tanker = Tanker(guests_monthly_wc)
    monthly_tanker_bill = tanker.calc_water_price()
    print(f"Month End Tanker Bill: Rs {monthly_tanker_bill}/-")

    print(f"Overall Water Capacity is {residents_monthly_wc + guests_monthly_wc} & Monthly Water Bill Found: Rs {int(monthly_coorp_bill + monthly_borewell_bill + monthly_tanker_bill)}/-")
    # print(f"{residents_monthly_wc + guests_monthly_wc} {int(monthly_coorp_bill + monthly_borewell_bill + monthly_tanker_bill)}")
